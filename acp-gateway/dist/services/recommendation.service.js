...
const pantry_recipe_service_1 = require("./pantry-recipe.service");
const report_service_1 = require("./report.service");
const user_profile_store_1 = require("./user-profile.store");
const SHORT_EXERCISE_CLIP_COUNT = 500;
const SHORT_CLIP_DURATIONS_SECONDS = [10, 12, 15, 18, 20, 22, 25, 28, 30];
const shortExerciseScenarios = [
    {
...
function buildShortExerciseClips(total) {
    const clips = [];
    let idx = 0;
    for (const scenario of shortExerciseScenarios) {
        for (const action of shortExerciseActions) {
            ...
            clips.push({
                ...
                durationSeconds,
                calories,
                tags: Array.from(new Set([...scenario.tags, ...action.tags, '基础动作', '动作片段', '运动视频'])),
...
const EXERCISE_INTENSITY_RANK = {
    low: 1,
    medium: 2,
    high: 3,
};
const RECIPE_DATA_FILE = path_1.default.resolve(__dirname, '../../../data/recipes.js');
...
const SIGNATURE_RECIPE_ORDER = new Map(SIGNATURE_RECIPE_IDS.map((id, index) => [id, SIGNATURE_RECIPE_IDS.length - index]));
const SIGNATURE_RECIPE_TRIGGER_TOPICS = new Set([
    '控糖',
    '控脂',
    '减脂',
    '高蛋白',
...
function loadRecipeDataFile() {
    try {
        if (!fs_1.default.existsSync(RECIPE_DATA_FILE)) {
            return [];
        }
        const source = fs_1.default.readFileSync(RECIPE_DATA_FILE, 'utf8');
        const sandbox = {};
        vm_1.default.runInNewContext(`${source}\nthis.__RECIPES__ = recipes;`, sandbox, { timeout: 1000 });
        return Array.isArray(sandbox.__RECIPES__)
            ? sandbox.__RECIPES__.filter((item) => item && typeof item === 'object')
            : [];
    }
    catch (error) {
...
function deriveRecipeRecommendationTags(recipe) {
    ...
    if (type === '早餐')
        add('均衡营养', '高蛋白');
    if (/低盐|少盐|高血压|血压|心血管/.test(combined))
        add('低盐', '心血管', '血压友好');
    if (/糖|燕麦|全麦|稳糖|控糖/.test(combined))
        add('控糖');
    if (/低脂|控脂|减脂|轻食/.test(combined))
        add('控脂', '减脂');
    if (/纤维|通便|蔬食|蔬菜/.test(combined))
        add('高纤维');
    if (/鸡胸|鸡蛋|豆腐|鱼|牛奶|虾|蛋白/.test(combined))
        add('高蛋白');
    if (/清淡|爽口|低负担|汤/.test(combined))
        add('清淡');
    if (/养胃|健脾|开胃|暖胃|消食/.test(combined))
        add('养胃');
    if (/免疫|抗疲劳|元气|滋补|补气|补血|养生|调理|恢复/.test(combined))
        add('恢复');
    if (/护肝|肝肾/.test(combined))
        add('护肝');
    if (/补肾/.test(combined))
        add('护肾');
    rawTags.forEach((tag) => {
        ...
        if (/清淡/.test(tag))
            add('清淡');
        if (/补气|养血|元气|滋补|免疫|抗疲劳/.test(tag))
            add('恢复');
        if (/补肾/.test(tag))
            add('护肾');
        if (/补肝|肝/.test(tag))
...
function buildRecipeContentsFromData(existingTitles) {
...
const existingRecipeTitles = new Set(contentCatalog
...
const CONTENT_STORE_DIR = path_1.default.resolve(__dirname, '../../uploads/content');
const RECIPES_STORE_FILE = path_1.default.join(CONTENT_STORE_DIR, 'recipes.json');
const EXERCISE_VIDEOS_STORE_FILE = path_1.default.join(CONTENT_STORE_DIR, 'exercise-videos.json');
const recipeSeedCatalog = contentCatalog
    .filter((content) => content.type === 'recipe')
    .map((content) => ({ ...content }));
const exerciseSeedCatalog = contentCatalog
    .filter((content) => content.type === 'exercise')
    .map((content) => ({ ...content }));
const staticContentCatalog = contentCatalog
    .filter((content) => content.type !== 'recipe' && content.type !== 'exercise')
    .map((content) => ({ ...content }));
function ensureContentStoreDir() {
    if (!fs_1.default.existsSync(CONTENT_STORE_DIR)) {
        fs_1.default.mkdirSync(CONTENT_STORE_DIR, { recursive: true });
    }
}
function writeJsonFile(filePath, payload) {
    fs_1.default.writeFileSync(filePath, JSON.stringify(payload, null, 2), 'utf8');
}
function ensureCatalogStoreFiles() {
    ensureContentStoreDir();
    if (!fs_1.default.existsSync(RECIPES_STORE_FILE)) {
        writeJsonFile(RECIPES_STORE_FILE, recipeSeedCatalog);
    }
    if (!fs_1.default.existsSync(EXERCISE_VIDEOS_STORE_FILE)) {
...
function readJsonArray(filePath) {
    try {
        ...
        console.warn(`读取内容库失败（${path_1.default.basename(filePath)}），将回退到默认内容：`, error);
        return null;
    }
}
...
function normalizeRecipeContent(raw) {
...
function normalizeExerciseIntensity(raw) {
    if (raw === 'low' || raw === 'medium' || raw === 'high') {
        return raw;
    }
    return 'low';
}
function normalizeExerciseContent(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const item = raw;
    const id = String(item.id || '').trim();
...
function loadStoredCatalog(filePath, fallback, normalize) {
...
function loadBackendContentCatalog() {
    return [
...
function addTopic(signals, topic, evidence, weight = 1) {
    signals.topics.add(topic);
    const existing = signals.topicEvidence.get(topic) ?? new Set();
...
function hasMeaningfulProfile(profile) {
    if (!profile)
        return false;
    return Boolean((profile.chronicDisease && profile.chronicDisease.length > 0) ||
...
function extractTopicsFromTextEntries(entries, signals, rules, evidencePrefix) {
...
function extractTopicsFromMessage(message, signals) {
...
function extractTopicsFromProfile(profile, signals) {
    if (!profile)
        return;
    ...
    for (const raw of diseases) {
        ...
        if (disease.includes('痛风'))
            addTopic(signals, '低嘌呤', '健康档案：痛风', 3.6);
    ...
    extractTopicsFromTextEntries(profile.symptoms, signals, [
    ...
    extractTopicsFromTextEntries(profile.healthGoals, signals, [
...
function extractTopicsFromIndicators(indicators, signals) {
    ...
    for (const item of indicators) {
        ...
        if (hypertensionPattern.test(combined))
            hasHypertensionLike = true;
        if (heartRiskPattern.test(combined))
            hasHeartRiskLike = true;
...
function isValidHealthReport(report) {
    if (!report)
        return false;
...
class RecommendationService {
    constructor() {
        this.recommendationHistoryByKey = new Map();
        this.recommendationCursorByKey = new Map();
    }
    async getRecommendations(scene, options = {}) {
    ...
    getRecommendationsSync(scene, userId, message = '') {
        const signals = {
            topics: new Set(),
        ...
        signals.pantryQuery = (0, pantry_recipe_service_1.buildPantryQueryResult)(message);
        if (signals.pantryQuery.isPantryQuery) {
            addTopic(signals, '饮食', '食材提问', 2.5);
        }
        if (userId) {
            const profile = user_profile_store_1.userProfiles.get(userId) ?? null;
    ...
    async buildSignals(scene, options) {
        const signals = {
            topics: new Set(),
            topicEvidence: new Map(),
            topicWeights: new Map(),
        ...
        const profile = options.profile !== undefined
            ? options.profile
            : options.userId
                ? (user_profile_store_1.userProfiles.get(options.userId) ?? null)
                : null;
        signals.hasProfile = hasMeaningfulProfile(profile);
        extractTopicsFromProfile(profile, signals);
        ...
        if (!report && options.userId) {
            try {
                const history = await report_service_1.reportService.getReportHistory(options.userId, 1, 1);
        ...
        if (isValidHealthReport(report)) {
            ...
            const { hasHypertensionLike, hasHeartRiskLike, hasSeriousAnomaly } = extractTopicsFromIndicators(anomalyIndicators, signals);
        ...
        if (signals.topics.size === 0) {
            if (scene === 'diet')
                addTopic(signals, '饮食', '场景默认', 0.8);
            if (scene === 'exercise')
                addTopic(signals, '运动', '场景默认', 0.8);
            if (scene === 'sleep')
                addTopic(signals, '助眠', '场景默认', 0.8);
            if (scene === 'psychology')
                addTopic(signals, '减压', '场景默认', 0.8);
        }
        return signals;
    }
    getPreferredContentTypes(scene, signals) {
        const hasReport = Boolean(signals.reportId);
        if (scene === 'diet')
            return hasReport ? ['recipe', 'exercise'] : ['recipe'];
        if (scene === 'exercise')
            return hasReport ? ['exercise', 'recipe'] : ['exercise'];
        if (scene === 'report')
            return ['recipe', 'exercise'];
        if (scene === 'sleep')
            return ['sleep', 'exercise'];
        if (scene === 'psychology')
            return ['psychology', 'exercise'];
        if (scene === 'antiaging')
            return ['exercise', 'recipe'];
        const topics = signals.topics;
        if (topics.has('助眠'))
            return ['sleep', 'exercise', 'recipe'];
        if (topics.has('减压'))
            return ['psychology', 'exercise', 'sleep'];
        if (topics.has('肩颈') ||
            topics.has('腰背') ||
            topics.has('久坐') ||
            topics.has('脖子抽筋') ||
    ...
    scoreContent(content, preferredTypes, signals) {
        let score = 0;
        ...
        return {
            score,
            matchedTopics: Array.from(matchedTopicSet).sort((a, b) => (signals.topicWeights.get(b) ?? 0) - (signals.topicWeights.get(a) ?? 0)),
        };
    }
    buildReason(matchedTopics, signals, fallback) {
        const picked = matchedTopics.slice(0, 2);
        const evidenceLabels = Array.from(new Set(picked.flatMap((topic) => Array.from(signals.topicEvidence.get(topic) ?? [])
    ...
    withResolvedUrl(content) {
        return {
            ...content,
            url: this.resolveContentUrl(content),
        };
    }
    buildPantryContents(query) {
        if (!query || !query.isPantryQuery)
            return [];
        const combined = [
    ...
    buildDiversityKey(scene, signals, userId) {
        const identity = String(userId || '').trim();
        if (identity)
            return `${scene}:user:${identity}`;
    ...
    getRecentRecommendationIds(diversityKey) {
        return new Set(this.recommendationHistoryByKey.get(diversityKey) ?? []);
    }
    rememberRecommendationIds(diversityKey, ids) {
        if (ids.length === 0)
            return;
    ...
    takeFromPoolWithCursor(pool, count, cursorKey, seenIds, selected) {
    ...
    pickRankedCandidates(ranked, count, options) {
        if (count <= 0)
            return [];
        const filtered = ranked.filter((item) => (!options.type || item.content.type === options.type) && !options.seenIds.has(item.content.id));
        ...
        if (selected.length < count) {
            this.takeFromPoolWithCursor(recycledPool, count - selected.length, `${keyBase}:recycled`, options.seenIds, selected);
        }
        if (selected.length < count) {
    ...
    shouldPrioritizeSignatureRecipes(scene, signals) {
        if (signals.pantryQuery?.isPantryQuery)
            return false;
        if (scene === 'diet')
            return true;
        if (scene !== 'chat' && scene !== 'report')
            return false;
        return Array.from(SIGNATURE_RECIPE_TRIGGER_TOPICS).some((topic) => signals.topics.has(topic));
    }
    getSignatureRecipePriority(item, signals) {
        const matchedWeight = item.matchedTopics.reduce((total, topic) => total + (signals.topicWeights.get(topic) ?? 0), 0);
    ...
    pickSignatureRecipes(ranked, count, seenIds, signals) {
        if (count <= 0)
            return [];
        const selected = [];
        const candidates = ranked
            .filter((item) => item.content.type === 'recipe' &&
            SIGNATURE_RECIPE_ORDER.has(item.content.id) &&
            !seenIds.has(item.content.id))
            .sort((a, b) => this.getSignatureRecipePriority(b, signals) - this.getSignatureRecipePriority(a, signals));
    ...
    toReasonedContents(items, signals, fallbackReason) {
        return items.map((item) => ({
            ...item.content,
            reason: this.buildReason(item.matchedTopics, signals, fallbackReason),
        }));
    }
    selectContents(scene, signals, userId) {
        const hasReport = Boolean(signals.reportId);
        ...
        const allowGenericFallback = scene === 'chat' ||
            scene === 'diet' ||
            scene === 'exercise' ||
            scene === 'sleep' ||
            scene === 'psychology' ||
        ...
        const scored = loadBackendContentCatalog()
            .map((c) => {
        ...
        const requireMatchedTopicsOnly = !hasReport &&
            !signals.hasProfile &&
        ...
        const pickedCandidates = (() => {
            if (!wantMix) {
                const signature = prioritizeSignatureRecipes
                    ...
                    : [];
                const remainder = signature.length < 3
            ...
            const secondaryType = preferredPrimary === 'recipe' ? 'exercise' : 'recipe';
            ...
            const primarySource = requireMatchedTopicsOnly && primaryMatchedCount < Math.max(primaryCount, 2) ? scored : ranked;
            const secondarySource = requireMatchedTopicsOnly && secondaryMatchedCount < 6 ? scored : ranked;
            const primarySignature = prioritizeSignatureRecipes && preferredPrimary === 'recipe'
        ...
        if (!hasRecipeRecommendation && deduped.length < 3) {
            const exerciseFallbacks = this.toReasonedContents(this.pickRankedCandidates(ranked, 3 - deduped.length, {
                diversityKey,
                recentIds,
                seenIds: seen,
                type: 'exercise',
            }), signals, personalizedFallback);
    ...
    getGoodsByScene(_scene, _signals) {
        return productCatalog.slice(0, 2);
    }
    getPagesByScene(_scene, _signals) {
        return pageCatalog;
    }
    async getContents() {
    ...
    async getStoredExerciseVideos() {
        return loadStoredExerciseVideos();
    }
    async saveStoredRecipes(items) {
        return replaceStoredRecipes(items);
    }
    async saveStoredExerciseVideos(items) {
