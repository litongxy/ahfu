...
const report_special_findings_library_1 = require("./report-special-findings-library");
...
const INDICATOR_SPECS = report_range_library_1.REPORT_RANGE_LIBRARY.map((item) => ({
...
class ReportService {
    async uploadAndAnalyze(userId, filePath, originalName) {
        ...
        try {
            ...
            const isParseFailed = report.indicators.some(i => i.name.includes('解析失败') ||
                i.name.includes('未找到') ||
                i.name.includes('请上传') ||
                i.name.includes('维护中') ||
...
[{"name":"总胆固醇","value":"实际数值","unit":"mmol/L"},{"name":"谷丙转氨酶","value":"实际数值","unit":"U/L"},...]`;
            const result = execSync(`~/.local/bin/opencode run ${JSON.stringify(prompt)} --image ${dataUrl} 2>/dev/null`, { encoding: 'utf-8', timeout: 60000, maxBuffer: 1024 * 1024 });
                            ...
                            if (nameKey) {
                                ...
                                if (!isNaN(value) && value > 0 && value < 2000) {
                                    ...
                                    indicators.push({
                                        ...
                                        isAnomaly,
                                        severity,
                                        suggestion: isAnomaly ? (0, report_model_1.generateAnomalySuggestions)({ name: info.name, value: String(value), unit: info.unit, referenceRange: info.range, isAnomaly, severity }) : '',
                                    });
                                }
                            }
                        }
                    ...
                    if (valueStr) {
                        ...
                        if (!isNaN(value) && value > 0 && range && value >= range.min && value <= range.max) {
                            ...
                            indicators.push({
                                ...
                                isAnomaly,
                                severity,
                                suggestion: isAnomaly ? (0, report_model_1.generateAnomalySuggestions)({ name: p.name, value: roundedValue, unit: p.unit, referenceRange: p.range, isAnomaly, severity }) : '',
                            });
                        }
                    }
                }
    ...
    deduplicateIndicators(indicators) {
        ...
        for (const indicator of indicators) {
            ...
            const shouldReplace = (indicator.isAnomaly && !existing.isAnomaly) ||
    ...
    extractQualitativeIndicators(lines) {
        return report_range_library_1.URINE_QUALITATIVE_LIBRARY
            .map(spec => this.extractQualitativeIndicator(lines, spec))
            .filter((item) => Boolean(item));
    }
    extractQualitativeIndicator(lines, spec) {
        const candidates = [];
        ...
        return {
            ...
            suggestion: evaluation.isAnomaly
                ? (0, report_model_1.generateAnomalySuggestions)({
                    name: spec.displayName,
                    value: best.value,
                    unit: '',
                    referenceRange: spec.normalRange,
    ...
    normalizeQualitativeValue(value) {
    ...
    evaluateQualitativeValue(value) {
        if (value === '阴性') {
    ...
    refineContextualIndicators(indicators) {
    ...
    normalizeCompatibilityText(text) {
        return text
    ...
    getSeverity(value, range) {
        ...
        if (min !== undefined && max !== undefined) {
            if (value < min * 0.5 || value > max * 2)
                return 'serious';
            if (value < min * 0.8 || value > max * 1.5)
                return 'abnormal';
    ...
    extractIndicatorFromLines(lines, spec) {
        const candidates = [];
        for (let index = 0; index < lines.length; index += 1) {
            const contexts = this.buildIndicatorContexts(lines, index);
            for (const line of contexts) {
                ...
                const hasStructuredEvidence = this.lineContainsUnit(line, spec.unit) ||
    ...
    buildIndicatorContexts(lines, index) {
    ...
    isWithinValidRange(value, range) {
        return !Number.isNaN(value) && value >= range.min && value <= range.max;
    }
    scoreIndicatorCandidate(line, spec) {
        let score = 0;
    ...
    extractSpecialFindings(lines) {
        const indicators = [];
        const ecgLines = Array.from(new Set(lines.flatMap((line, index) => {
            if (!/心电图/.test(line)) {
                return [];
            }
    ...
    extractRuleBasedFindings(lines) {
        return report_special_findings_library_1.SPECIAL_FINDING_LIBRARY
            .map(rule => this.extractRuleBasedFinding(lines, rule))
            .filter((item) => Boolean(item));
    }
    extractRuleBasedFinding(lines, rule) {
        const candidates = [];
        for (let index = 0; index < lines.length; index += 1) {
    ...
    buildFindingContexts(lines, index) {
        const contexts = new Set();
        const current = lines[index];
        const previous = index > 0 ? lines[index - 1] : '';
        const next = index < lines.length - 1 ? lines[index + 1] : '';
    ...
    containsNegativeFindingContext(line, rule) {
        return [...report_special_findings_library_1.DEFAULT_NEGATIVE_FINDING_PATTERNS, ...(rule.negativeAliases || [])].some(pattern => {
            pattern.lastIndex = 0;
            return pattern.test(line);
        });
    }
    extractRuleBasedFindingValue(line, fallbackValue) {
    ...
    resolveRuleBasedSeverity(line, defaultSeverity) {
    ...
    applyThyroidPatternSuggestions(indicators) {
    ...
    applyInflammationSuggestions(indicators) {
    ...
    looksLikeHealthReport(text) {
        const normalized = text.toLowerCase();
        const reportMarkers = ['体检报告', 'medical examination report', '健康体检报告'];
        if (reportMarkers.some(marker => normalized.includes(marker))) {
            return true;
        }
        const strongLabKeywords = ['血糖', '糖化血红蛋白', '尿酸', '肌酐', '白细胞', '红细胞', '血红蛋白', '血小板', '胆固醇', '甘油三酯', '转氨酶', '心电图', '尿蛋白', '尿糖', '尿潜血', '尿白细胞', '促甲状腺激素', 'ft3', 'ft4', 'c反应蛋白', '血沉', '降钙素原', ...report_special_findings_library_1.SPECIAL_FINDING_HEALTH_KEYWORDS];
        return strongLabKeywords.filter(keyword => normalized.includes(keyword.toLowerCase())).length >= 2;
    }
    getHealthKeywordHitCount(text) {
        const healthKeywords = ['体检', '报告', '血糖', '糖化血红蛋白', '尿酸', '肌酐', '白细胞', '红细胞', '血红蛋白', '血小板', '胆固醇', '甘油三酯', '转氨酶', '心电图', '尿蛋白', '尿糖', '尿潜血', '尿白细胞', '促甲状腺激素', 'ft3', 'ft4', 'c反应蛋白', '血沉', '降钙素原', ...report_special_findings_library_1.SPECIAL_FINDING_HEALTH_KEYWORDS];
        const normalized = text.toLowerCase();
        return healthKeywords.filter(keyword => normalized.includes(keyword.toLowerCase())).length;
    }
    extractEcgFindingValue(line) {
        const normalized = this.normalizeReportLine(line)
            .replace(/^.*?心电图[:：]?\s*/, '')
            .replace(/^检查所见[:：]?\s*/, '')
            .trim();
    ...
    createParseFailureIndicators(message) {
        return [{
                name: '⚠️ 解析失败',
                value: '未检测到有效指标',
                unit: '',
    ...
    async analyzeWithAI(indicators, userId) {
...
- 【你还可以补充的信息】`;
        try {
            const result = (0, child_process_1.execSync)(`~/.local/bin/opencode run ${JSON.stringify(prompt)} 2>/dev/null`, { encoding: 'utf-8', timeout: 60000, maxBuffer: 1024 * 1024 });
    ...
    generateDefaultAnalysis(anomalyItems) {
            ...
            const severityLabel = i.severity === 'serious'
                ...
                : i.severity === 'abnormal'
                    ...
                    : i.severity === 'slight'
                        ? '轻度异常'
                        : '异常';
            const suggestion = i.suggestion || (0, report_model_1.generateAnomalySuggestions)(i);
    ...
    async getReportHistory(userId, page = 1, pageSize = 10) {
        const userReports = [];
    ...
    async generatePDFReport(reportId) {
        ...
        try {
            ...
            if (report.aiAnalysis) {
                const analysisY = doc.lastAutoTable?.finalY || 120;
                doc.setFontSize(14);
