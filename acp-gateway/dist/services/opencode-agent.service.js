...
exports.opencodeAgent = exports.OpencodeAgentService = void 0;
...
const page_route_service_1 = require("./page-route.service");
const questions_knowledge_service_1 = require("./questions-knowledge.service");
...
function buildPantryRecipeSuggestion(message) {
    return (0, pantry_recipe_service_1.buildPantryRecipeSuggestion)(message);
}
function fetchHealthProfile(userId) {
    return user_profile_store_1.userProfiles.get(userId) || null;
}
function saveHealthProfile(profile) {
    profiles.set(profile.userId, profile);
}
function formatHealthProfileForPrompt(profile) {
    if (!profile)
        return '';
    const parts = [];
...
function checkMedicationContraindications(userMessage, profile) {
    if (!profile)
        return '';
...
function generatePersonalizedSuggestions(userMessage, profile) {
    if (!profile)
        return '';
    const msg = userMessage.toLowerCase();
    const suggestions = [];
...
class OpencodeAgentService {
    constructor() {
        this.knowledgeService = new knowledge_service_1.KnowledgeService();
    ...
    async initialize() {
        try {
            // 测试 opencode 是否可用
            const result = (0, child_process_1.execSync)('~/.local/bin/opencode run "测试" 2>/dev/null', {
                encoding: 'utf-8',
                timeout: 5000
            });
    ...
    async chat(message, context) {
        ...
        const healthReminders = generateHealthReminders(profile ?? {});
        let response = '';
                ...
                if (!hasMedicalKnowledgeHit && this.shouldPreferWebSearch(message, scene)) {
                    ...
                    if (this.useAI) {
                        try {
                            ...
                            response = questions_knowledge_service_1.questionsKnowledge.getDirectAnswer(message) || this.getFallbackResponse(message, scene);
                        }
                    }
                    else {
                        ...
                        response = knowledgeResults.length > 0
                            ...
                            : (questions_knowledge_service_1.questionsKnowledge.getDirectAnswer(message) || this.getFallbackResponse(message, scene));
                    }
        ...
        const pageRoutes = page_route_service_1.pageRouteService.getRecommendedRoutes(message);
    ...
    shouldRecommendProfileAndReport(message, response, scene) {
        const trimmedMessage = message.trim();
        if (!trimmedMessage)
            return false;
        const lower = trimmedMessage.toLowerCase();
        if (this.isSmallTalk(lower) || lower.includes('谢谢'))
            return false;
        const trimmedResponse = response.trim();
        if (!trimmedResponse)
            return true;
        if (trimmedResponse === this.getOutOfScopeResponse().trim())
            return true;
        if (scene !== 'chat')
            return false;
    ...
    getProfileAndReportPages() {
        const ids = ['health-profile', 'report-analysis'];
        return ids
            .map((id) => page_route_service_1.pageRouteService.getRouteById(id))
            .filter((route) => Boolean(route))
            .map((route) => ({ name: route.name, url: route.url, icon: route.icon }));
    }
    async callAI(message, profileContext = '', extraContext = '') {
        const knowledgeContext = questions_knowledge_service_1.questionsKnowledge.getRelevantContext(message);
        const parts = [SYSTEM_PROMPT, knowledgeContext, profileContext, extraContext]
            .map((part) => part.trim())
            .filter(Boolean);
        const prompt = `${parts.join('\n\n')}\n\n用户问题：${message}`;
        const result = (0, child_process_1.execSync)(`~/.local/bin/opencode run ${JSON.stringify(prompt)} 2>/dev/null`, { encoding: 'utf-8', timeout: 30000, maxBuffer: 1024 * 1024 });
    ...
    detectScene(message) {
        ...
        if (m.includes('运动') || m.includes('健身') || m.includes('跑步') || m.includes('锻炼') || m.includes('减肥'))
            return 'exercise';
        if (m.includes('心理') || m.includes('压力') || m.includes('情绪') || m.includes('焦虑'))
            return 'psychology';
        if (m.includes('睡眠') || m.includes('失眠') || m.includes('睡觉') || m.includes('入睡'))
            return 'sleep';
        if (m.includes('抗衰') || m.includes('美容') || m.includes('保养'))
            return 'antiaging';
        if (m.includes('同仁堂') || m.includes('品牌') || m.includes('历史'))
            return 'brand';
        if (m.includes('体检') || m.includes('报告') || m.includes('指标'))
            return 'report';
        if (m.includes('中医') ||
            m.includes('养生') ||
            m.includes('调理') ||
    ...
    isOutOfScope(message, scene) {
        ...
        if (this.isSmallTalk(m))
            return false;
        if (this.looksLikeHealthQuestion(m))
            return false;
        if (this.hasMedicalKnowledgeHit(trimmed))
            return false;
        return true;
    }
    hasMedicalKnowledgeHit(message) {
        const trimmed = message.trim();
        if (!trimmed)
            return false;
        return questions_knowledge_service_1.questionsKnowledge.search(trimmed, 1).length > 0;
    }
    shouldPreferWebSearch(message, scene) {
        if (!web_search_service_1.webSearchService.isEnabled())
            return false;
        const trimmed = message.trim();
        if (!trimmed)
            return false;
        ...
        if (this.isSmallTalk(lower))
            return false;
        if (this.isCookingRecipeIntent(lower))
            return false;
        if (!this.looksLikeHealthQuestion(lower))
            return false;
        if (scene === 'brand')
            return false;
        return !this.hasMedicalKnowledgeHit(trimmed);
    }
    async tryAnswerWithWebSearch(message, profileContext) {
        if (!web_search_service_1.webSearchService.isEnabled())
            return null;
        const results = await web_search_service_1.webSearchService.search(message, { maxResults: 5 });
        if (results.length === 0)
            return null;
        try {
            return this.useAI
    ...
    getMissingKnowledgeResponse(searchAttempted) {
        if (searchAttempted) {
...
仅供参考，如有不适请就医。`;
    ...
    getFallbackResponse(message, scene) {
        const m = message.toLowerCase();
...
仅供参考，如有不适请就医。`,
    ...
    shouldTriggerWebSearch(message, response, scene) {
        if (!web_search_service_1.webSearchService.isEnabled())
            return false;
        const trimmedMessage = message.trim();
        if (!trimmedMessage)
        ...
        if (m.includes('谢谢'))
            return false;
        if (this.isCookingRecipeIntent(m))
            return false;
        const trimmedResponse = response.trim();
        if (!trimmedResponse)
            return true;
        ...
        if (uncertainPhrases.some((p) => trimmedResponse.includes(p)))
            return true;
        if (this.looksLikeHealthQuestion(m) && !this.hasMedicalKnowledgeHit(trimmedMessage))
            return true;
        const fallback = this.getFallbackResponse(trimmedMessage, scene).trim();
        if (trimmedResponse === fallback)
            return true;
        if (trimmedMessage.length >= 10 && trimmedResponse.length < 30)
            return true;
        return false;
    }
    isCookingRecipeIntent(message) {
    ...
    getCookingIntentFallback(message) {
        const ingredients = (0, pantry_recipe_service_1.extractIngredientsFromMessage)(message);
    ...
    formatWebResultsForPrompt(results) {
    ...
    formatWebSearchAnswer(message, results) {
        const top = results.slice(0, 5);
...
exports.OpencodeAgentService = OpencodeAgentService;
exports.opencodeAgent = new OpencodeAgentService();
