...
function parseIngredientBlock(block: string): string[] {
...
function toDefinitions(items: string[], category: IngredientCategory): IngredientAliasDefinition[] {
...
const BASE_DEFINITIONS: IngredientAliasDefinition[] = [
...
function buildDerivedDefinitions(definitions: IngredientAliasDefinition[]): IngredientAliasDefinition[] {
  const derived: IngredientAliasDefinition[] = [];
...
const ALL_DEFINITIONS: IngredientAliasDefinition[] = [...BASE_DEFINITIONS, ...buildDerivedDefinitions(BASE_DEFINITIONS)];

const CATEGORY_PREFIXES: Record<IngredientCategory, string[]> = {
  vegetable: ['新鲜', '鲜'],
  mushroom: ['鲜', '干'],
  fish: ['鲜', '冷冻'],
  shellfish: ['鲜', '冷冻'],
...
function buildAutoAliases(definition: IngredientAliasDefinition): string[] {
  const canonical = definition.canonical;
  const set = new Set<string>([canonical, ...(definition.aliases || [])]);
...
export function buildExtendedIngredientAliasMap(): Record<string, string[]> {
  ...
  ALL_DEFINITIONS.forEach((definition) => {
    ...
    if (!map.has(canonical)) {
      map.set(canonical, new Set<string>());
    }
...
export function getExtendedIngredientCatalogStats(): { canonicalCount: number; termCount: number } {
