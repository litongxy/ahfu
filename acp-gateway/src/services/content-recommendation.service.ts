...
export class ContentRecommendationService {
  private contents = contentDatabase;
  private products = productDatabase;

  getContents(scene: string, limit: number = 3): Content[] {
    return this.contents
      .filter(c => c.targetScenes.includes(scene))
      .slice(0, limit);
  }

  getProducts(scene: string, limit: number = 2): Product[] {
    return this.products
      .filter(p => p.targetScenes.includes(scene))
      .slice(0, limit);
  }

  searchContents(keyword: string, limit: number = 5): Content[] {
  ...
  searchProducts(keyword: string, limit: number = 5): Product[] {
    const lower = keyword.toLowerCase();
    return this.products
      .filter(p => 
