...
function sign(input: string): string {
  const digest = crypto.createHmac('sha256', getTokenSecret()).update(input).digest();
...
export class AuthTokenService {
  createToken(input: CreateTokenInput): { token: string; claims: AuthTokenClaims; expiresInSeconds: number } {
  ...
  verifyToken(token: string): AuthTokenClaims | null {
    ...
    try {
      const header = JSON.parse(decodeBase64Url(encodedHeader)) as AuthTokenHeader;
      if (header.alg !== 'HS256' || header.typ !== 'JWT') {
        return null;
      }

      const claims = JSON.parse(decodeBase64Url(encodedPayload)) as AuthTokenClaims;
      const nowSeconds = Math.floor(Date.now() / 1000);
