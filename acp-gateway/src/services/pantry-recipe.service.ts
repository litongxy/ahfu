import { buildExtendedIngredientAliasMap } from '../data/extended-ingredient-catalog';
...
type RecipeDraft = Omit<PantryRecipe, 'id'> & { id?: string };
...
const PANTRY_CORE_INGREDIENT_ALIASES: Record<string, string[]> = {
  番茄: ['番茄', '西红柿', '西红柿块', '番茄块'],
  鸡蛋: ['鸡蛋', '蛋', '鸭蛋'],
...
const PANTRY_INGREDIENT_NOISE_PATTERN =
  /([0-9]+(?:\.[0-9]+)?\s*(?:kg|g|ml|l|克|千克|毫升|升|斤)|[0-9]+(?:\.[0-9]+)?)|适量|少许|少量|若干|一把|适中|备用|即可|汤匙|茶匙|大勺|小勺|勺|把|个|根|片|朵|颗|只|块|碗|滴|杯/g;
const PANTRY_INGREDIENT_ACTION_PATTERN =
  /洗净|切块|切片|切丝|切段|切丁|焯水|提前浸泡|提前泡发|泡发|去皮|去核|去根|拍碎|打散|腌制|浸泡|备好|备用|切末/g;
...
const PANTRY_EXTENDED_INGREDIENT_ALIASES = buildExtendedIngredientAliasMap();

const PANTRY_BASE_INGREDIENT_ALIASES = mergeIngredientAliasMaps(
  PANTRY_EXTENDED_INGREDIENT_ALIASES,
  PANTRY_CORE_INGREDIENT_ALIASES
);

const pantryBaseAliasLookup = buildAliasLookup(
  PANTRY_EXTENDED_INGREDIENT_ALIASES,
  PANTRY_CORE_INGREDIENT_ALIASES
);
...
function mergeIngredientAliasMaps(...sources: Array<Record<string, string[]>>): Record<string, string[]> {
  const merged = new Map<string, Set<string>>();
  sources.forEach((source) => {
    Object.entries(source || {}).forEach(([canonical, aliases]) => {
      if (!merged.has(canonical)) {
        merged.set(canonical, new Set<string>());
      }
...
function buildAliasLookup(...sources: Array<Record<string, string[]>>): Map<string, string> {
...
function canonicalizeIngredient(raw: string): string {
...
function normalizeRecipeIngredients(ingredients: string[]): string[] {
...
const PANTRY_SEED_RECIPES: RecipeDraft[] = [
...
const stirPartnerDisplay: Record<string, string> = {
  鸡蛋: '鸡蛋',
  豆腐: '豆腐',
  猪肉: '肉片',
  牛肉: '牛肉',
...
const staplePartnerDisplay: Record<string, string> = {
  鸡蛋: '鸡蛋',
  豆腐: '豆腐',
  猪肉: '肉片',
  牛肉: '牛肉',
...
const fillerStirStyles: FillerStyle[] = [
...
const fillerStapleStyles: FillerStyle[] = [
...
const fillerSoupStyles: FillerStyle[] = [
...
function registerRecipe(recipeMap: Map<string, PantryRecipe>, draft: RecipeDraft): void {
...
function buildGeneratedPantryRecipes(): PantryRecipe[] {
  const recipeMap = new Map<string, PantryRecipe>();
  PANTRY_SEED_RECIPES.forEach((recipe) => registerRecipe(recipeMap, recipe));

  coldVegetables.forEach((veg) => {
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`凉拌${veg}`, '凉菜', '5分钟', [veg], '清爽开胃，单一主食材也能快速成菜。', ['凉菜', '快手'], 90)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}拌木耳`, '凉菜', '8分钟', [veg, '木耳'], '脆爽口感更丰富，适合夏季。', ['凉菜', '爽口'], 70)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}拌豆腐`, '凉菜', '8分钟', [veg, '豆腐'], '清淡高蛋白，轻食感更强。', ['凉菜', '轻食'], 68)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}拌粉丝`, '凉菜', '10分钟', [veg, '粉丝'], '简单顶饱，适合做凉拌小菜。', ['凉菜', '爽口'], 66)
    );
  });
  ...
  quickVegetables.forEach((veg) => {
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`清炒${veg}`, '素食', '8分钟', [veg], '做法直接，适合想快点上桌的场景。', ['素食', '快手'], 62)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`蒜蓉${veg}`, '素食', '8分钟', [veg], '蒜香思路常见，操作难度低。', ['素食', '家常'], 61)
    );
  });
  ...
  eggFriendlyVegetables.forEach((veg) => {
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}炒鸡蛋`, '家常菜', '12分钟', [veg, '鸡蛋'], '经典双拼思路，厨房容错率高。', ['家常', '快手'], 74)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}鸡蛋汤`, '汤类', '10分钟', [veg, '鸡蛋'], '热汤快手方案，适合搭配米饭。', ['汤类', '清淡'], 66)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}鸡蛋饼`, '早餐', '15分钟', [veg, '鸡蛋'], '早餐友好，饱腹感更稳。', ['早餐', '快手'], 58)
    );
  });
  ...
  tofuFriendlyVegetables.forEach((veg) => {
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}烧豆腐`, '家常菜', '15分钟', [veg, '豆腐'], '清淡里带点下饭感，家常很常见。', ['家常', '豆制品'], 70)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}豆腐汤`, '汤类', '12分钟', [veg, '豆腐'], '清爽顺口，晚餐搭配很合适。', ['汤类', '清淡'], 64)
    );
    registerRecipe(
      recipeMap,
      buildRecipeDraft(`${veg}炒豆腐`, '家常菜', '12分钟', [veg, '豆腐'], '双素菜思路稳定，也方便补蛋白。', ['家常', '轻食'], 63)
    );
  });
    ...
    staplePartners.forEach((partner) => {
      fillerStapleStyles.forEach((style) => {
        registerRecipe(
          recipeMap,
          buildRecipeDraft(
            `${veg}${staplePartnerDisplay[partner] || partner}${style.suffix}`,
            style.type,
            style.time,
            [veg, partner, style.suffix === '炒饭' || style.suffix === '盖饭' ? '米饭' : '挂面'],
            style.highlight,
            ['主食', veg, partner],
            style.priority
          )
    ...
    stirPartners.forEach((partner) => {
      fillerStirStyles.forEach((style) => {
        registerRecipe(
          recipeMap,
          buildRecipeDraft(
            `${veg}${stirPartnerDisplay[partner] || partner}${style.suffix}`,
            style.type,
            style.time,
            [veg, partner],
            style.highlight,
...
export const PANTRY_RECIPES: PantryRecipe[] = buildGeneratedPantryRecipes();
...
const pantryAliasEntries = Array.from(pantryBaseAliasLookup.entries())
  ...
  .filter((item) => {
    ...
    if (item.alias.length >= 2) return true;
...
export function extractIngredientsFromMessage(message: string): string[] {
  ...
  const sorted = Array.from(found).sort(
    (a, b) => normalizeIngredientToken(b).length - normalizeIngredientToken(a).length
  );
...
export function isPantryQuery(message: string, ingredients: string[]): boolean {
...
export function findPantryMatches(ingredients: string[]): { full: PantryMatch[]; partial: PantryMatch[] } {
  ...
  const partial = scored
    ...
    .sort(
      (a, b) =>
        b.hitCount - a.hitCount || a.missing.length - b.missing.length || b.score - a.score || a.name.localeCompare(b.name, 'zh-Hans-CN')
    )
    .slice(0, 10);
...
export function buildPantryQueryResult(message: string): PantryQueryResult {
...
export function buildPantryRecipeSuggestion(message: string): string | null {
  ...
  if (!query.isPantryQuery) return null;
  ...
  const lines: string[] = [];
  lines.push(`你现在有「${ownedText}」，我先按现有食材给你筛了一版。当前食材库已支持 ${PANTRY_RECIPES.length} 道常见菜搭配：`);
...
function buildGenericDishIdeas(ingredients: string[]): string[] {
  const ideas: string[] = [];
...
export function getPantryRecipeLibraryStats(): { count: number; ingredientAliasCount: number } {
  return {
    count: PANTRY_RECIPES.length,
