import { ReportIndicator } from './report.model';

type Severity = 'normal' | 'slight' | 'abnormal' | 'serious';
...
const severityLabel = (severity: Severity): string => {
  if (severity === 'serious') return '重点关注';
  if (severity === 'abnormal') return '异常';
  if (severity === 'slight') return '轻度异常';
  return '正常';
};

const DEPT_RULES: Array<{
  match: RegExp;
  dept: string;
  followUp: string;
}> = [
...
const inferDeptAdvice = (indicator: ReportIndicator): { dept: string; followUp: string } | null => {
...
const formatIndicatorLine = (indicator: ReportIndicator): string => {
...
export function buildNoAnomalyAdvice(): string {
  return [
    '【总体评估】',
...
export function buildAnomalyAdvice(anomalyItems: ReportIndicator[]): string {
  const sorted = [...anomalyItems].sort((a, b) => {
    const rank = (s: Severity) => (s === 'serious' ? 3 : s === 'abnormal' ? 2 : s === 'slight' ? 1 : 0);
