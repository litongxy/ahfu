import { BLOOD_RANGE_BASELINES, URINE_RANGE_BASELINES } from './report-range-library';
...
export const BLOOD_INDICATORS = BLOOD_RANGE_BASELINES;
...
export const URINE_INDICATORS = URINE_RANGE_BASELINES;

export function parseValue(valueStr: string, normalRange: string): { value: number; isAnomaly: boolean; severity: 'normal' | 'slight' | 'abnormal' | 'serious' } {
...
export function generateAnomalySuggestions(indicator: ReportIndicator): string {
  const name = indicator.name.toLowerCase();

  if (name.includes('脂肪肝')) return '建议复查腹部B超，注意饮食调理';
  if (name.includes('谷丙转氨酶') || name.includes('(alt)')) return '建议复查肝功能，避免饮酒并规律作息';
  if (name.includes('谷草转氨酶') || name.includes('(ast)')) return '建议复查肝功能，并结合心肌酶等检查综合判断';
  if (name.includes('碱性磷酸酶') || name.includes('(alp)')) return '建议结合肝胆和彩超结果，排查胆道或骨代谢问题';
  if (name.includes('总胆红素') || name.includes('(tbil)')) return '建议结合肝功能及胆道检查，必要时复查胆红素分型';
  if (name.includes('尿蛋白')) return '建议复查尿常规与肾功能，必要时就诊肾内科';
  if (name.includes('尿糖')) return '建议结合空腹血糖或糖化血红蛋白，评估糖代谢情况';
  if (name.includes('尿酮体')) return '建议关注进食情况、血糖变化，必要时复查尿酮体';
  if (name.includes('尿潜血')) return '建议复查尿常规，必要时排查泌尿系统出血或结石';
  if (name.includes('尿白细胞')) return '建议结合尿培养或泌尿系统症状，排查感染';
  if (name.includes('空腹血糖') || name.includes('(glu)')) return '建议复查空腹血糖，必要时做糖耐量试验';
  if (name.includes('糖化血红蛋白')) return '建议结合空腹血糖与饮食习惯，评估近三个月血糖控制情况';
  if (name.includes('总胆固醇') || name.includes('(tc)')) return '注意控制饱和脂肪摄入，规律运动并复查血脂';
  if (name.includes('甘油三酯') || name.includes('(tg)')) return '建议控制精制碳水和饮酒，增加运动并复查血脂';
  if (name.includes('高密度脂蛋白') || name.includes('(hdl)')) return '建议结合整体血脂结果评估心血管风险';
  if (name.includes('低密度脂蛋白') || name.includes('(ldl)')) return '建议控制饮食并结合心血管风险进行管理';
  ...
  if (name.includes('ft4')) return '建议结合 TSH、FT3 综合判断甲状腺功能状态';
  if (name.includes('c反应蛋白') || name.includes('(crp)')) return '提示炎症或感染活动，建议结合症状和血常规综合判断';
  if (name.includes('血沉') || name.includes('(esr)')) return '提示炎症或免疫活动可能，建议结合 CRP 和临床症状评估';
  if (name.includes('降钙素原') || name.includes('(pct)')) return '提示细菌感染可能，建议结合体温、血常规和医生评估';
